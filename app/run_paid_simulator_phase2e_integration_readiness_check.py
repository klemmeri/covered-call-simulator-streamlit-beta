"""
run_paid_simulator_phase2e_integration_readiness_check.py

Phase 2E controlled premium-adjustment integration-readiness check for the
Covered Call Strategy Stress Test project.

This script verifies that the Phase 2E adjustment layer is installed, that
its outputs are present, and that the main dashboard contains the
Developer-view-only Phase 2E adjustment tab markers.
...
whether the adjusted premium assumptions are financially correct.
"""
...
REPORT_PATH = OUTPUT_REPORTS_DIR / "phase2e_integration_readiness_report.txt"
...
class CheckResult:
    ...
    def line(self) -> str:
        if self.detail:
            return f"{self.status:<10} {self.label:<58} {self.detail}"
...
def main() -> int:
    ...
    lines: list[str] = []
    lines.append(banner("Phase 2E controlled premium-adjustment integration-readiness check"))
    ...
    source_files = [
        ...
        ("Phase 2E adjustment viewer", PAID_SIMULATOR_DIR / "phase2e_adjustment_viewer.py"),
    ...
    lines.append(section("Required Phase 2E source files"))
    ...
    runner_files = [
        ...
        ("Adjusted premium payoff comparison check", APP_DIR / "run_paid_simulator_adjusted_premium_payoff_comparison_check.py"),
        ("Phase 2E adjustment viewer launcher", APP_DIR / "run_paid_simulator_phase2e_adjustment_viewer.py"),
        ("Phase 2E adjustment viewer check", APP_DIR / "run_paid_simulator_phase2e_adjustment_viewer_check.py"),
        ("Phase 2E adjustment pipeline check", APP_DIR / "run_paid_simulator_phase2e_adjustment_pipeline_check.py"),
        ("Phase 2E dashboard-tab check", APP_DIR / "run_paid_simulator_phase2e_dashboard_tab_check.py"),
    ]

    lines.append(section("Required Phase 2E runner/check files"))
    ...
    lines.append(section("Required Phase 2E config files"))
    ...
    generated_outputs = [
        ...
        ("Premium adjustment comparison HTML", OUTPUT_REPORTS_DIR / "premium_model_adjustment_comparison.html"),
        ("Premium adjustment summary", OUTPUT_REPORTS_DIR / "premium_model_adjustment_summary.txt"),
        ("Adjusted premium payoff comparison CSV", OUTPUT_TABLES_DIR / "adjusted_premium_payoff_comparison.csv"),
        ("Adjusted premium payoff comparison HTML", OUTPUT_REPORTS_DIR / "adjusted_premium_payoff_comparison.html"),
        ("Adjusted premium payoff summary", OUTPUT_REPORTS_DIR / "adjusted_premium_payoff_summary.txt"),
        ("Phase 2E adjustment pipeline report", OUTPUT_REPORTS_DIR / "phase2e_adjustment_pipeline_report.txt"),
    ...
    lines.append(section("Expected Phase 2E generated outputs"))
    ...
    documentation_files = [
        ("Phase 2E premium model adjustment", DOCS_DIR / "paid_simulator_phase2e_premium_model_adjustment.md"),
        ...
        ("Phase 2E adjustment pipeline", DOCS_DIR / "paid_simulator_phase2e_adjustment_pipeline_check.md"),
    ...
    lines.append(section("Phase 2E documentation"))
    results = [check_exists(label, path) for label, path in documentation_files]
    all_results.extend(results)
    add_results(lines, results)

    lines.append(section("Main dashboard Phase 2E markers"))
    results = check_marker(
        "Dashboard marker",
        PAID_SIMULATOR_DIR / "config_form_app.py",
        [
            "Phase 2E adjustment",
            "premium_model_adjusted_premiums",
            "adjusted_premium_payoff_comparison",
    ...
    lines.append("")
    lines.append(banner(f"Overall Phase 2E integration-readiness status: {status}"))
    if status == "PASS":
        lines.append("Phase 2E controlled premium adjustment is installed and connected. The project is ready for the Phase 2E checkpoint.")
    else:
        lines.append("One or more Phase 2E integration-readiness items need attention before the Phase 2E checkpoint.")
