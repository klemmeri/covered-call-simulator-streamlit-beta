...
Phase 2E controlled premium-adjustment pipeline checker for the Covered Call
Strategy Stress Test.

This script does not modify the customer-facing dashboard. It runs the Phase 2E
adjustment workflow and verifies that the adjusted-premium and adjusted-payoff
comparison outputs are being produced.
"""
...
REPORT_PATH = REPORT_DIR / "phase2e_adjustment_pipeline_report.txt"
...
def main() -> int:
    ...
    script_steps = [
        ScriptStep(
            name="Phase 2D checkpoint check",
            path=APP_DIR / "run_paid_simulator_phase2d_checkpoint_check.py",
            required=False,
        ),
        ScriptStep(
            name="Phase 2E controlled premium adjustment check",
            path=APP_DIR / "run_paid_simulator_premium_model_adjustment_check.py",
            required=True,
        ),
        ScriptStep(
            name="Phase 2E adjusted premium payoff comparison check",
            path=APP_DIR / "run_paid_simulator_adjusted_premium_payoff_comparison_check.py",
            required=True,
        ),
        ScriptStep(
            name="Phase 2E adjustment viewer check",
            path=APP_DIR / "run_paid_simulator_phase2e_adjustment_viewer_check.py",
            required=True,
        ),
    ]
    ...
    file_checks = [
        FileCheck(
            label="Premium-model adjustment config",
            path=CONFIG_DIR / "premium_model_adjustment_config.json",
            kind="json",
        ),
        FileCheck(
            label="Original option-premium scaffold CSV",
        ...
        FileCheck(
            label="Adjusted premium CSV",
            path=TABLE_DIR / "premium_model_adjusted_premiums.csv",
            kind="csv",
        ),
        FileCheck(
            label="Adjusted premium comparison HTML",
            path=REPORT_DIR / "premium_model_adjustment_comparison.html",
            kind="html",
        ),
        FileCheck(
            label="Adjusted premium summary text",
            path=REPORT_DIR / "premium_model_adjustment_summary.txt",
            kind="text",
        ),
        FileCheck(
            label="Adjusted premium payoff comparison CSV",
            path=TABLE_DIR / "adjusted_premium_payoff_comparison.csv",
            kind="csv",
        ),
        FileCheck(
            label="Adjusted premium payoff comparison HTML",
            path=REPORT_DIR / "adjusted_premium_payoff_comparison.html",
            kind="html",
        ),
        FileCheck(
            label="Adjusted premium payoff summary text",
            path=REPORT_DIR / "adjusted_premium_payoff_summary.txt",
            kind="text",
        ),
        FileCheck(
            label="Phase 2E premium adjustment check report",
            path=REPORT_DIR / "phase2e_premium_model_adjustment_check_report.txt",
            kind="text",
        ),
        FileCheck(
            label="Phase 2E adjusted payoff check report",
            path=REPORT_DIR / "phase2e_adjusted_premium_payoff_comparison_check_report.txt",
            kind="text",
        ),
        FileCheck(
            label="Phase 2E adjustment viewer check report",
            path=REPORT_DIR / "phase2e_adjustment_viewer_check_report.txt",
            kind="text",
        ),
    ]
    ...
    marker_checks = [
        MarkerCheck(
            label="Adjusted premium summary",
            path=REPORT_DIR / "premium_model_adjustment_summary.txt",
            markers=[
                "premium",
                "adjustment",
    ...
    lines.append("=" * 96)
    lines.append("Phase 2E controlled premium-adjustment pipeline check")
    lines.append("=" * 96)
    ...
    lines.append("Expected Phase 2E files and outputs")
    lines.append("-" * 96)
    ...
    lines.append("-" * 96)
    lines.append(f"Overall Phase 2E adjustment pipeline status: {overall}")
    lines.append("")

    if overall == "PASS":
        lines.append(
            "The Phase 2E controlled adjustment workflow is installed, runnable, "
            "and producing the expected adjusted-premium and adjusted-payoff outputs."
        )
    else:
        lines.append(
            "One or more required Phase 2E adjustment-pipeline items failed. Review "
            "the messages above before adding the Phase 2E adjustment layer to the main dashboard."
        )
